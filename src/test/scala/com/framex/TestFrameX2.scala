...
class TestFrameX2 extends FlatSpec with Matchers {
  ...
  it should "init FrameX from two dimensional list" in {
    ...
    val df2 = df(2)
    df(2).equals(FrameX2(ll2)) shouldEqual true
    df(2, 4).equals(FrameX2(ll24)) shouldEqual true
  ...
  it should "return head" in {
    ...
    FrameX2(ll, columnNames).head() shouldBe FrameX2(
      List(
        List(1, 2, 3, 4, 5),
        List('a', 'b', 'c', 'd', 'e')
      ), columnNames
  ...
  it should "return head n" in {
    val ll = List(List.range(1, 27),
      List.range('a', '{'))
    val columnNames = List("id", "word")
    FrameX2(ll, columnNames).head(3) shouldBe FrameX2(
      List(
        List(1, 2, 3),
        List('a', 'b', 'c')
      ),
  ...
  it should "return head not overflow" in {
    val ll = List(List.range(1, 6),
      List.range('a', 'f'))
    val columnNames = List("id", "word")
    FrameX2(ll, columnNames).head(10) shouldBe FrameX2(ll, columnNames)
  }
  ...
  it should "return tail" in {
    ...
    FrameX2(ll, columnNames).tail() shouldBe FrameX2(
      Vector
      (
  ...
  it should "return tail n" in {
    val ll = List(List(1, 2, 3, 4, 5),
      ...
      List("2015-01-10", "2017-08-22", "2016-03-03", "2011-02-02", "2017-02-12"))
    val columnNames = List("id", "word", "date")
    FrameX2(ll, columnNames).tail(3) shouldBe FrameX2(
      Vector(
    ...
    FrameX2(ll, columnNames).tail(1) shouldBe FrameX2(
      Vector(
        Vector(ElemX(5)),
        Vector(ElemX("E")),
  ...
  it should "return tail not overflow" in {
    val ll = List(List(1, 2, 3, 4, 5),
      ...
      List("2015-01-10", "2017-08-22", "2016-03-03", "2011-02-02", "2017-02-12"))
    val columnNames = List("id", "word", "date")
    FrameX2(ll, columnNames).tail(10) shouldBe FrameX2(
      Vector
      (
  ...
  it should "FrameX with different columnName / columnIndex should not equal" in {
    ...
    (FrameX2(ll, columnNames1) != FrameX2(ll, columnNames2)) shouldEqual (true)
    (FrameX2(ll, columnNames1) != FrameX2(ll, columnNames3)) shouldEqual (true)
  }
  ...
  it should "append new frameX to existed frameX" in {
    ...
    val existedDf = FrameX2(existedLL, columnNames)
    val appendedDf = FrameX2(appendedLL, columnNames)
    val df = FrameX2(ll, columnNames)
  ...
  it should "groupBy one column" in {
    ...
    val df = FrameX2(ll, columnNames)
    val testDataMap: Map[String, FrameX2] = Map(
      List(ElemX(1)).toString() -> FrameX2(List(
        List(1, 1),
        List(1, 2),
        List(0.36, 0.22)
      ), columnNames),
      List(ElemX(2)).toString() -> FrameX2(List(
        List(2, 2),
        List(3, 4),
        List(1.26, -0.56)
      ), columnNames)
  ...
  it should "groupBy multi columns" in {
    ...
    val columnNames = List("a", "b", "c")
    val testedDataMap: Map[String, FrameX2] = Map(
      List(ElemX(1), ElemX(1)).toString() -> FrameX2(List(
        List(1, 1), List(1, 1), List('a', 'b')
      ), columnNames),
      List(ElemX(4), ElemX(4)).toString() -> FrameX2(List(
        List(4), List(4), List('f')
      ), columnNames),
      List(ElemX(3), ElemX(3)).toString() -> FrameX2(List(
        List(3), List(3), List('e')
      ), columnNames),
      List(ElemX(2), ElemX(2)).toString() -> FrameX2(List(
        List(2, 2), List(2, 2), List('c', 'd')
      ), columnNames),
      List(ElemX(5), ElemX(5)).toString() -> FrameX2(List(
  ...
  it should "agg functions after groupBy FrameX" in {
    ...
    val frameAfterGroupByA1 = FrameX2(List(List(2), List(3), List(0.58)))
    ...
    val frameAfterGroupByA2 = FrameX2(List(List(4), List(7), List(0.7)))
